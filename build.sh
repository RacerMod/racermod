...
if [ "$2" = "kernel" ]; then
    ...
    echo -e $CL_GRN"Moving boot.img to racermod folder..."$CL_RST
    rm ${racermod}/cm7/${product}/boot/gen2_boot.img
    mv ${android}/out/target/product/${device}/boot.img ${racermod}/cm7/${product}/boot/gen2_boot.img
    echo -e $CL_GRN"Done!"$CL_RST
...
echo -e $CL_GRN"Copying boot folder..."$CL_RST
mkdir temp
cp -r boot temp/boot
echo -e $CL_GRN"Done!"$CL_RST
...
if [ "$1" = "blade" ]; then
    ${racermod}/mkbootimg --base 0x02A00000 --cmdline 'androidboot.hardware=blade console=null' --kernel gen1zImage --ramdisk ramdisk.gz -o temp/boot/gen1_boot.img
else
